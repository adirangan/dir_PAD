...
% Using statistics from Chimpanzee data. ;
% e.g.,: dt_avg = 1.5; %<-- Chimpanzee data for typical pair of measurements. ;
% n_i = 450; %<-- roughly matches Chimpanzee data. ;
% n_j_factor = 1.0; %<-- matches Chimpanzee data. ;
% ignore_factor = 0.01; %<-- low missingness. ;
% T_ini =  0;
% T_max = 22.5; %<-- roughly 15 samples per chimpanzee. ;
%%%%%%%%;
%%%%%%%%;
str_thisfunction = 'test_Chimpanzee_4';
...
%%%%;
parameter_Chimpanzee = struct('type','parameter');
parameter_Chimpanzee.dir_trunk = dir_trunk;
parameter_Chimpanzee.n_i = n_i;
parameter_Chimpanzee.B_log_amplitude = B_log_amplitude;
parameter_Chimpanzee.C_log_amplitude = C_log_amplitude;
parameter_Chimpanzee.n_iteration_BtBn = n_iteration_BtBn;
parameter_Chimpanzee.MaxFunEvals_use_BtBn = MaxFunEvals_use_BtBn;
parameter_Chimpanzee.MaxFunEvals_use_simultaneous = MaxFunEvals_use_simultaneous;
parameter_Chimpanzee.rseed = rseed;
test_Chimpanzee_4(parameter_Chimpanzee);
%%%%;
...
%%%%%%%%;
na=0;
if (nargin<1+na); parameter_Chimpanzee=[]; end; na=na+1;
%%%%%%%%;
if isempty(parameter_Chimpanzee); parameter_Chimpanzee = struct('type','parameter'); end;
if ~isfield(parameter_Chimpanzee,'dir_trunk'); parameter_Chimpanzee.dir_trunk = pwd; end;
if ~isfield(parameter_Chimpanzee,'dir_Chimpanzee_mat'); parameter_Chimpanzee.dir_Chimpanzee_mat = sprintf('%s/dir_Chimpanzee_mat',parameter_Chimpanzee.dir_trunk); end;
if ~isfield(parameter_Chimpanzee,'tolerance_master'); parameter_Chimpanzee.tolerance_master = 1e-6; end;
if ~isfield(parameter_Chimpanzee,'flag_verbose'); parameter_Chimpanzee.flag_verbose = 1; end;
if ~isfield(parameter_Chimpanzee,'flag_disp'); parameter_Chimpanzee.flag_disp = 0; end;
if ~isfield(parameter_Chimpanzee,'dt_avg'); parameter_Chimpanzee.dt_avg = 1.0; end;
if ~isfield(parameter_Chimpanzee,'n_i'); parameter_Chimpanzee.n_i = 450; end;
if ~isfield(parameter_Chimpanzee,'n_j_factor'); parameter_Chimpanzee.n_j_factor = 1.00; end;
if ~isfield(parameter_Chimpanzee,'ignore_factor'); parameter_Chimpanzee.ignore_factor = 0.01; end;
if ~isfield(parameter_Chimpanzee,'T_ini'); parameter_Chimpanzee.T_ini = 0.00; end;
if ~isfield(parameter_Chimpanzee,'T_max'); parameter_Chimpanzee.T_max = 15.0; end;
if ~isfield(parameter_Chimpanzee,'B_log_amplitude'); parameter_Chimpanzee.B_log_amplitude = 0.0; end;
if ~isfield(parameter_Chimpanzee,'C_log_amplitude'); parameter_Chimpanzee.C_log_amplitude = 0.0; end;
if ~isfield(parameter_Chimpanzee,'X_log_amplitude'); parameter_Chimpanzee.X_log_amplitude = 0.0; end;
if ~isfield(parameter_Chimpanzee,'n_iteration_BtBn'); parameter_Chimpanzee.n_iteration_BtBn = 32; end;
if ~isfield(parameter_Chimpanzee,'MaxFunEvals_use_BtBn'); parameter_Chimpanzee.MaxFunEvals_use_BtBn = 1024; end;
if ~isfield(parameter_Chimpanzee,'MaxFunEvals_use_simultaneous'); parameter_Chimpanzee.MaxFunEvals_use_simultaneous = 0; end;
if ~isfield(parameter_Chimpanzee,'flag_regularize_eccentricity_simultaneous'); parameter_Chimpanzee.flag_regularize_eccentricity_simultaneous = 1; end;
if ~isfield(parameter_Chimpanzee,'rseed'); parameter_Chimpanzee.rseed = 1; end;
%%%%%%%%;
dir_trunk = parameter_Chimpanzee.dir_trunk;
dir_Chimpanzee_mat = parameter_Chimpanzee.dir_Chimpanzee_mat;
tolerance_master = parameter_Chimpanzee.tolerance_master;
flag_verbose = parameter_Chimpanzee.flag_verbose;
flag_disp = parameter_Chimpanzee.flag_disp;
dt_avg = parameter_Chimpanzee.dt_avg;
n_i = parameter_Chimpanzee.n_i;
n_j_factor = parameter_Chimpanzee.n_j_factor;
ignore_factor = parameter_Chimpanzee.ignore_factor;
T_ini = parameter_Chimpanzee.T_ini;
T_max = parameter_Chimpanzee.T_max;
B_log_amplitude = parameter_Chimpanzee.B_log_amplitude;
C_log_amplitude = parameter_Chimpanzee.C_log_amplitude;
X_log_amplitude = parameter_Chimpanzee.X_log_amplitude;
n_iteration_BtBn = parameter_Chimpanzee.n_iteration_BtBn;
MaxFunEvals_use_BtBn = parameter_Chimpanzee.MaxFunEvals_use_BtBn;
MaxFunEvals_use_simultaneous = parameter_Chimpanzee.MaxFunEvals_use_simultaneous;
flag_regularize_eccentricity_simultaneous = parameter_Chimpanzee.flag_regularize_eccentricity_simultaneous;
rseed = parameter_Chimpanzee.rseed;
%%%%%%%%;
...
if ~exist(dir_Chimpanzee_mat,'dir'); disp(sprintf(' %% mkdir %s',dir_Chimpanzee_mat)); mkdir(dir_Chimpanzee_mat); end;
tmp_dir = sprintf('%s/dir_test_Chimpanzee_%s',dir_Chimpanzee_mat,dir_infix);
if ~exist(tmp_dir,'dir') disp(sprintf(' %% mkdir %s',tmp_dir)); mkdir(tmp_dir); end;
tmp_fname_mat_ini = sprintf('%s/test_Chimpanzee_%s.mat',dir_Chimpanzee_mat,str_infix);
tmp_fname_mat_fin = sprintf('%s/dir_test_Chimpanzee_%s/test_Chimpanzee_%s.mat',dir_Chimpanzee_mat,dir_infix,str_infix);
if  exist(tmp_fname_mat_ini,'file'); 
disp(sprintf(' %% mv %s %s',tmp_fname_mat_ini,tmp_fname_mat_fin)); movefile(tmp_fname_mat_ini,tmp_fname_mat_fin);
end;%if  exist(tmp_fname_mat_ini,'file'); 
fname_pre = sprintf('%s/dir_test_Chimpanzee_%s/test_Chimpanzee_%s',dir_Chimpanzee_mat,dir_infix,str_infix);
