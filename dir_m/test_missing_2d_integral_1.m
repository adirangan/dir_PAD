function test_missing_2d_integral_1(rseed);
...
%%%%;
P = @(XY1,XY2) 1/(2*pi) * sqrt(det(CtCn_xx__)) * exp(-0.5*[XY1 , XY2]*CtCn_xx__*[XY1;XY2]);
%%%%%%%%;
n_xy1 = 1024*1.0 + 0;
xy_max = 7.5;
...
+ (exp(C_l1) - exp(C_l0))*sin(2*C_omega)*tmp_z ...
 ;
[Z2_base,l2_stretch] = SDE_missing_2d_integral_helper_0(1,C_omega,C_l0,C_l1);
%%%%;
p_B_ = zeros(n_xy1,1);
...
- (exp(C_l1) - exp(C_l0))*sin(2*C_omega)*tmp_z ...
 ;
[Z2_base,l2_stretch] = SDE_missing_2d_integral_helper_0(0,C_omega,C_l0,C_l1);
%%%%;
p_B_ = zeros(n_xy2,1);
...
p_B = 1/sqrt(2*pi) * det(isigma__)/max(1e-12,sqrt(l2_stretch)) * exp(-Z2/2);
p_B_(1+nxy2) = p_B;
nlp_B_(1+nxy2) = 0.5*log(2*pi) - 0.5*(C_l0 + C_l1) + 0.5*log(l2_stretch) + 0.5*Z2_base*XY2.^2;
end;%for nxy2=0:n_xy2-1;
%%%%%%%%;
disp(sprintf(' %% p_A_ vs p_B_: %0.16f',fnorm(p_A_-p_B_)./fnorm(p_A_)));
disp(sprintf(' %% nlp_A_ vs nlp_B_: %0.16f',fnorm(nlp_A_-nlp_B_)./fnorm(nlp_A_)));
