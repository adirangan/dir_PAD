...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Now check for tmp_fname_collect_mat. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
n_x = 2;
...
tmp_fname_collect_mat = sprintf('%s.mat',tmp_fname_collect_pre);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
if ~exist(tmp_fname_collect_mat,'file');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
end;%if ~exist(tmp_fname_collect_mat,'file');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
tmp_fname_collect_mat = sprintf('%s.mat',tmp_fname_collect_pre);
if  exist(tmp_fname_collect_mat,'file');
disp(sprintf(' %% found %s, not creating',tmp_fname_collect_mat));
tmp_ = load(tmp_fname_collect_mat);

AZ_cmb_xxvv____ = zeros(n_x,n_x,n_var,n_var);
AZ_cmb_01_vv__ = zeros(n_var,n_var);
AZ_cmb_10_vv__ = zeros(n_var,n_var);
AZ_cmb_vv__ = zeros(n_var,n_var);
A_cmb_xxvv____ = zeros(n_x,n_x,n_var,n_var);
A_cmb_01_vv__ = zeros(n_var,n_var);
A_cmb_10_vv__ = zeros(n_var,n_var);
A_cmb_vv__ = zeros(n_var,n_var);
for nv0=0:n_var-1;
for nv1=nv0+1:n_var-1;
tmp_A_xxs___ = tmp_.A_est_xxsvv_____(:,:,:,1+nv0,1+nv1);
tmp_A_ori_xx__ = tmp_A_xxs___(:,:,1+0);
tmp_A_avg_xx__ = mean(tmp_A_xxs___(:,:,2:end),3);
tmp_A_std_xx__ = std(tmp_A_xxs___(:,:,2:end),1,3);
tmp_AZ_ori_xx__ = (tmp_A_ori_xx__ - tmp_A_avg_xx__)./max(1e-12,tmp_A_std_xx__);
AZ_cmb_xxvv____(:,:,1+nv0,1+nv1) = tmp_AZ_ori_xx__;
AZ_cmb_01_vv__(1+nv0,1+nv1) = tmp_AZ_ori_xx__(1+0,1+1);
AZ_cmb_10_vv__(1+nv0,1+nv1) = tmp_AZ_ori_xx__(1+1,1+0);
AZ_cmb_vv__(1+nv0,1+nv1) = AZ_cmb_01_vv__(1+nv0,1+nv1);
AZ_cmb_vv__(1+nv1,1+nv0) = AZ_cmb_10_vv__(1+nv0,1+nv1);
A_cmb_xxvv____(:,:,1+nv0,1+nv1) = tmp_A_ori_xx__;
A_cmb_01_vv__(1+nv0,1+nv1) = tmp_A_ori_xx__(1+0,1+1);
A_cmb_10_vv__(1+nv0,1+nv1) = tmp_A_ori_xx__(1+1,1+0);
A_cmb_vv__(1+nv0,1+nv1) = A_cmb_01_vv__(1+nv0,1+nv1);
A_cmb_vv__(1+nv1,1+nv0) = A_cmb_10_vv__(1+nv0,1+nv1);
end;%for nv1=nv0+1:n_var-1;
end;%for nv0=0:n_var-1;
figure(1+nf);nf=nf+1;clf;figbig;fig80s;
fontsize_use = 8;
subplot(1,2,1);
imagesc(AZ_cmb_vv__,6*[-1,+1]); 
axis image;
set(gca,'XTick',1:n_var,'XTickLabel',tmp_.u_label_measurement_); xtickangle(90);
set(gca,'YTick',1:n_var,'YTickLabel',tmp_.u_label_measurement_);
set(gca,'TickLength',[0,0]);
title('ZA');
subplot(1,2,2);
imagesc(A_cmb_vv__,0.1*[-1,+1]); 
axis image;
set(gca,'XTick',1:n_var,'XTickLabel',tmp_.u_label_measurement_); xtickangle(90);
set(gca,'YTick',1:n_var,'YTickLabel',tmp_.u_label_measurement_);
set(gca,'TickLength',[0,0]);
...
BtBnZ_cmb_xxvv____ = zeros(n_x,n_x,n_var,n_var);
BtBnZ_cmb_01_vv__ = zeros(n_var,n_var);
BtBnZ_cmb_10_vv__ = zeros(n_var,n_var);
BtBnZ_cmb_vv__ = zeros(n_var,n_var);
BtBn_cmb_xxvv____ = zeros(n_x,n_x,n_var,n_var);
BtBn_cmb_01_vv__ = zeros(n_var,n_var);
BtBn_cmb_10_vv__ = zeros(n_var,n_var);
BtBn_cmb_vv__ = zeros(n_var,n_var);
for nv0=0:n_var-1;
for nv1=nv0+1:n_var-1;
tmp_BtBn_xxs___ = tmp_.BtBn_est_xxsvv_____(:,:,:,1+nv0,1+nv1);
tmp_BtBn_ori_xx__ = tmp_BtBn_xxs___(:,:,1+0);
tmp_BtBn_avg_xx__ = mean(tmp_BtBn_xxs___(:,:,2:end),3);
tmp_BtBn_std_xx__ = std(tmp_BtBn_xxs___(:,:,2:end),1,3);
tmp_BtBnZ_ori_xx__ = (tmp_BtBn_ori_xx__ - tmp_BtBn_avg_xx__)./max(1e-12,tmp_BtBn_std_xx__);
BtBnZ_cmb_xxvv____(:,:,1+nv0,1+nv1) = tmp_BtBnZ_ori_xx__;
BtBnZ_cmb_01_vv__(1+nv0,1+nv1) = tmp_BtBnZ_ori_xx__(1+0,1+1);
BtBnZ_cmb_10_vv__(1+nv0,1+nv1) = tmp_BtBnZ_ori_xx__(1+1,1+0);
BtBnZ_cmb_vv__(1+nv0,1+nv1) = BtBnZ_cmb_01_vv__(1+nv0,1+nv1);
BtBnZ_cmb_vv__(1+nv1,1+nv0) = BtBnZ_cmb_10_vv__(1+nv0,1+nv1);
BtBn_cmb_xxvv____(:,:,1+nv0,1+nv1) = tmp_BtBn_ori_xx__;
BtBn_cmb_01_vv__(1+nv0,1+nv1) = tmp_BtBn_ori_xx__(1+0,1+1);
BtBn_cmb_10_vv__(1+nv0,1+nv1) = tmp_BtBn_ori_xx__(1+1,1+0);
BtBn_cmb_vv__(1+nv0,1+nv1) = BtBn_cmb_01_vv__(1+nv0,1+nv1);
BtBn_cmb_vv__(1+nv1,1+nv0) = BtBn_cmb_10_vv__(1+nv0,1+nv1);
end;%for nv1=nv0+1:n_var-1;
end;%for nv0=0:n_var-1;
figure(1+nf);nf=nf+1;clf;figbig;fig80s;
fontsize_use = 8;
subplot(1,2,1);
imagesc(BtBnZ_cmb_vv__,6*[-1,+1]); 
axis image;
set(gca,'XTick',1:n_var,'XTickLabel',tmp_.u_label_measurement_); xtickangle(90);
set(gca,'YTick',1:n_var,'YTickLabel',tmp_.u_label_measurement_);
set(gca,'TickLength',[0,0]);
...
CtCnZ_cmb_xxvv____ = zeros(n_x,n_x,n_var,n_var);
CtCnZ_cmb_01_vv__ = zeros(n_var,n_var);
CtCnZ_cmb_10_vv__ = zeros(n_var,n_var);
CtCnZ_cmb_vv__ = zeros(n_var,n_var);
CtCn_cmb_xxvv____ = zeros(n_x,n_x,n_var,n_var);
CtCn_cmb_01_vv__ = zeros(n_var,n_var);
CtCn_cmb_10_vv__ = zeros(n_var,n_var);
CtCn_cmb_vv__ = zeros(n_var,n_var);
for nv0=0:n_var-1;
for nv1=nv0+1:n_var-1;
tmp_CtCn_xxs___ = tmp_.CtCn_est_xxsvv_____(:,:,:,1+nv0,1+nv1);
tmp_CtCn_ori_xx__ = tmp_CtCn_xxs___(:,:,1+0);
tmp_CtCn_avg_xx__ = mean(tmp_CtCn_xxs___(:,:,2:end),3);
tmp_CtCn_std_xx__ = std(tmp_CtCn_xxs___(:,:,2:end),1,3);
tmp_CtCnZ_ori_xx__ = (tmp_CtCn_ori_xx__ - tmp_CtCn_avg_xx__)./max(1e-12,tmp_CtCn_std_xx__);
CtCnZ_cmb_xxvv____(:,:,1+nv0,1+nv1) = tmp_CtCnZ_ori_xx__;
CtCnZ_cmb_01_vv__(1+nv0,1+nv1) = tmp_CtCnZ_ori_xx__(1+0,1+1);
CtCnZ_cmb_10_vv__(1+nv0,1+nv1) = tmp_CtCnZ_ori_xx__(1+1,1+0);
CtCnZ_cmb_vv__(1+nv0,1+nv1) = CtCnZ_cmb_01_vv__(1+nv0,1+nv1);
CtCnZ_cmb_vv__(1+nv1,1+nv0) = CtCnZ_cmb_10_vv__(1+nv0,1+nv1);
CtCn_cmb_xxvv____(:,:,1+nv0,1+nv1) = tmp_CtCn_ori_xx__;
CtCn_cmb_01_vv__(1+nv0,1+nv1) = tmp_CtCn_ori_xx__(1+0,1+1);
CtCn_cmb_10_vv__(1+nv0,1+nv1) = tmp_CtCn_ori_xx__(1+1,1+0);
CtCn_cmb_vv__(1+nv0,1+nv1) = CtCn_cmb_01_vv__(1+nv0,1+nv1);
CtCn_cmb_vv__(1+nv1,1+nv0) = CtCn_cmb_10_vv__(1+nv0,1+nv1);
end;%for nv1=nv0+1:n_var-1;
end;%for nv0=0:n_var-1;
figure(1+nf);nf=nf+1;clf;figbig;fig80s;
fontsize_use = 8;
subplot(1,2,1);
imagesc(CtCnZ_cmb_vv__,6*[-1,+1]); 
axis image;
set(gca,'XTick',1:n_var,'XTickLabel',tmp_.u_label_measurement_); xtickangle(90);
set(gca,'YTick',1:n_var,'YTickLabel',tmp_.u_label_measurement_);
set(gca,'TickLength',[0,0]);
...
nv0 = efind(strcmp(tmp_.u_label_measurement_,'Red Blood Cells'));
nv1 = efind(strcmp(tmp_.u_label_measurement_,'Mean Corpuscular Volume'));

nv0 = efind(strcmp(tmp_.u_label_measurement_,'Lymphocytes-%'));
nv1 = efind(strcmp(tmp_.u_label_measurement_,'Lymphocytes-Abs'));
