...
str_thisfunction = 'SDE3d_nlp_ijXaABYC_update_all_2';
nf=0;
...
if (nargin<1+na); B_est_omega_=[]; end; na=na+1;
if (nargin<1+na); B_est_l0=[]; end; na=na+1;
if (nargin<1+na); B_est_l1=[]; end; na=na+1;
if (nargin<1+na); B_est_l2=[]; end; na=na+1;
...
if (nargin<1+na); C_est_omega_=[]; end; na=na+1;
if (nargin<1+na); C_est_l0=[]; end; na=na+1;
if (nargin<1+na); C_est_l1=[]; end; na=na+1;
if (nargin<1+na); C_est_l2=[]; end; na=na+1;
...
[~,BtBn_est_xx__] =SDE3d_BtBn_1([],B_est_omega_,B_est_l0,B_est_l1,B_est_l2);
[~,CtCn_est_xx__] =SDE3d_BtBn_1([],C_est_omega_,C_est_l0,C_est_l1,C_est_l2);
disp(sprintf(' %% B_est_omega_ [%+0.2f,%+0.2f,%+0.2f] B_est_l0 %+0.2f B_est_l1 %+0.2f B_est_l2 %+0.2f',B_est_omega_,B_est_l0,B_est_l1,B_est_l2));
disp(sprintf(' %% C_est_omega_ [%+0.2f,%+0.2f,%+0.2f] C_est_l0 %+0.2f C_est_l1 %+0.2f C_est_l2 %+0.2f',C_est_omega_,C_est_l0,C_est_l1,C_est_l2));
...
B_est_omega_ = B_tmp_omega_;
B_est_l0 = B_tmp_l0;
B_est_l1 = B_tmp_l1;
B_est_l2 = B_tmp_l2;
C_est_omega_ = C_tmp_omega_;
C_est_l0 = C_tmp_l0;
C_est_l1 = C_tmp_l1;
C_est_l2 = C_tmp_l2;
...
[~,BtBn_est_xx__] =SDE3d_BtBn_1([],B_est_omega_,B_est_l0,B_est_l1,B_est_l2);
[~,CtCn_est_xx__] =SDE3d_BtBn_1([],C_est_omega_,C_est_l0,C_est_l1,B_est_l2);
disp(sprintf(' %% B_est_omega_ [%+0.2f,%+0.2f,%+0.2f] B_est_l0 %+0.2f B_est_l1 %+0.2f B_est_l2 %+0.2f',B_est_omega_,B_est_l0,B_est_l1,B_est_l2));
disp(sprintf(' %% C_est_omega_ [%+0.2f,%+0.2f,%+0.2f] C_est_l0 %+0.2f C_est_l1 %+0.2f C_est_l2 %+0.2f',C_est_omega_,C_est_l0,C_est_l1,C_est_l2));
if isfield(parameter_est,'nlp_ijXYC_pos'); disp(sprintf(' %% niteration %d/%d: nlp_ijXYC_pre %0.6f --> nlp_ijXYC_pos %0.6f',niteration,n_iteration,parameter_est.nlp_ijXYC_pre,parameter_est.nlp_ijXYC_pos)); nlp_ijXYC_l_(1+niteration) = parameter_est.nlp_ijXYC_pos; end;
if isfield(parameter_est,'nlp_idtZPB_pos'); disp(sprintf(' %% niteration %d/%d: nlp_idtZPB_pre %0.6f --> nlp_idtZPB_pos %0.6f',niteration,n_iteration,parameter_est.nlp_idtZPB_pre,parameter_est.nlp_idtZPB_pos)); nlp_idtZPB_l_(1+niteration) = parameter_est.nlp_idtZPB_pos; end;
...
tmp_W_xt__ = X_est_ixt___{1+ni}; tmp_str = 'X_est_xt__';
subplot(p_row,p_col,1+pcol+prow*p_col);
s = surfline_0(tmp_W_xt__(1,:),tmp_W_xt__(2,:),tmp_W_xt__(3,:),t_t_); set(s,'LineWidth',3);
axis vis3d;
xlabel(sprintf('%s(1+0,:)',tmp_str),'Interpreter','none');
ylabel(sprintf('%s(1+1,:)',tmp_str),'Interpreter','none');
zlabel(sprintf('%s(1+2,:)',tmp_str),'Interpreter','none');
title(sprintf('ni %d',ni),'Interpreter','none');
end;%for pcol=0:p_col-1;
%%%%;
