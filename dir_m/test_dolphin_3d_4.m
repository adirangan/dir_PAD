...
%%%%%%%%;
% Built from test_SDE3d_nlp_ijXaABYC_update_2.m. ;
...
%}
%%%%%%%%;
str_thisfunction = 'test_dolphin_3d_4';
...
test_dolphin_3d_4(parameter_dolphin);
%%%%;
...
if ~isfield(parameter_dolphin,'dir_dolphin_mat'); parameter_dolphin.dir_dolphin_mat = sprintf('%s/dir_dolphin_3d_mat',parameter_dolphin.dir_trunk); end;
...
tmp_dir = sprintf('%s/dir_test_dolphin_3d_%s',dir_dolphin_mat,dir_infix);
if ~exist(tmp_dir,'dir') disp(sprintf(' %% mkdir %s',tmp_dir)); mkdir(tmp_dir); end;
tmp_fname_mat_ini = sprintf('%s/test_dolphin_3d_%s.mat',dir_dolphin_mat,str_infix);
tmp_fname_mat_fin = sprintf('%s/dir_test_dolphin_3d_%s/test_dolphin_3d_%s.mat',dir_dolphin_mat,dir_infix,str_infix);
...
fname_pre = sprintf('%s/dir_test_dolphin_3d_%s/test_dolphin_3d_%s',dir_dolphin_mat,dir_infix,str_infix);
...
BidW_L2_avg = (exp(-B_tru_l0) + exp(-B_tru_l1) + exp(-B_tru_l2))*dt_avg;
Cinv_L2_avg = (exp(-C_tru_l0) + exp(-C_tru_l1) + exp(-C_tru_l2))*1;
%{
  %%%%;
  ...
  tmp2 = (exp(-1*B_tru_l0)+exp(-1*B_tru_l1)+exp(-1*B_tru_l2))*dt_avg;
...
B_est_omega_=zeros(n_x,1);
B_est_l0=-10;
B_est_l1=-10;
B_est_l2=-10;
C_est_omega_=zeros(n_x,1);
C_est_l0=-10;
C_est_l1=-10;
C_est_l2=-10;
%%%%;
...
SDE3d_nlp_ijXaABYC_update_all_2( ...
 parameter_est ...
,n_i ...
,n_t_i_ ...
,t_it__ ...
...
plot(t_t_,tmp_W_xt__(1,:),'r-',t_t_,tmp_W_xt__(2,:),'b-',t_t_,tmp_W_xt__(3,:),'g-');
...
if prow==1; tmp_W_xt__ = X_est_ixt___{1+ni}; tmp_str = 'X_est_xt__'; end;
subplot(p_row,p_col,1+pcol+prow*p_col);
s = surfline_0(tmp_W_xt__(1,:),tmp_W_xt__(2,:),tmp_W_xt__(3,:),t_t_); set(s,'LineWidth',3); axis vis3d;
